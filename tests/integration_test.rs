...
mod tests {
    ...
    #[tokio::test]
    async fn test_admin_principal_list_returns_stable_pagination_metadata() {
        let server = setup_test_server().await;
        let database_url = std::env::var("TEST_DATABASE_URL")
            .expect("TEST_DATABASE_URL is required for principal pagination integration tests");
        let pool = db::init_db_pool(&database_url).await.unwrap();
        db::run_migrations(&pool).await.unwrap();
        ...
        db::create_user(
            &pool,
            &format!("principal-pagination-a-{suffix}"),
            &format!("principal-pagination-a-{suffix}@example.test"),
            Some("PrincipalA#123"),
        )
        .await
        .unwrap();
        ...
        db::create_user(
            &pool,
            &format!("principal-pagination-b-{suffix}"),
            &format!("principal-pagination-b-{suffix}@example.test"),
            Some("PrincipalB#123"),
        )
        .await
        .unwrap();
        ...
        let first = server
            .get("/v1/admin/principals?principal_type=user&limit=1&offset=0")
            .add_header("Authorization", format!("Bearer {admin_token}"))
            .await;
        first.assert_status_ok();
        let first_body: serde_json::Value = first.json();
        ...
        let second = server
            .get("/v1/admin/principals?principal_type=user&limit=1&offset=1")
            .add_header("Authorization", format!("Bearer {admin_token}"))
            .await;
        second.assert_status_ok();
        let second_body: serde_json::Value = second.json();
