...
pub struct RuntimeMetrics {
    ...
    database_readiness_successes_total: AtomicU64,
    database_readiness_failures_total: AtomicU64,
...
impl RuntimeMetrics {
    pub fn new() -> Self {
        Self {
            ...
            database_readiness_successes_total: AtomicU64::new(0),
            database_readiness_failures_total: AtomicU64::new(0),
    ...
    /// Record an authentication request rejected by the configured rate limiter.
    pub fn rate_limit_rejection_observed(&self) {
        self.rate_limit_rejections_total
            .fetch_add(1, Ordering::Relaxed);
    }

    /// Records one database readiness result and its duration without host or query labels.
    pub fn database_readiness_observed(&self, duration_milliseconds: u64, succeeded: bool) {
        record_duration(
        ...
        readiness_outcome_observed(
            succeeded,
            &self.database_readiness_successes_total,
            &self.database_readiness_failures_total,
        );
    }

    /// Records one Redis connection-and-ping readiness result without leaking host or key details.
    pub fn redis_readiness_observed(&self, duration_milliseconds: u64, succeeded: bool) {
        record_duration(
            duration_milliseconds,
        ...
        readiness_outcome_observed(
            succeeded,
            &self.redis_readiness_successes_total,
            &self.redis_readiness_failures_total,
        );
    }
    ...
    pub fn prometheus_text(&self) -> String {
        format!(
            ...
            self.database_readiness_duration_milliseconds_count.load(Ordering::Relaxed),
            self.database_readiness_successes_total.load(Ordering::Relaxed),
            self.database_readiness_failures_total.load(Ordering::Relaxed),
            ...
            self.redis_readiness_duration_milliseconds_count.load(Ordering::Relaxed),
            self.redis_readiness_successes_total.load(Ordering::Relaxed),
            self.redis_readiness_failures_total.load(Ordering::Relaxed),
        )
    }
}
...
/// Increments the fixed success or failure counter for one dependency readiness probe.
fn readiness_outcome_observed(succeeded: bool, successes: &AtomicU64, failures: &AtomicU64) {
    if succeeded {
...
mod tests {
    ...
    fn prometheus_metrics_count_authentication_and_authorization_outcomes() {
        ...
        metrics.database_readiness_observed(12, true);
        ...
        assert!(text.contains("keylo_database_readiness_duration_milliseconds_count 2"));
        assert!(text.contains("keylo_database_readiness_successes_total 1"));
        assert!(text.contains("keylo_database_readiness_failures_total 1"));
