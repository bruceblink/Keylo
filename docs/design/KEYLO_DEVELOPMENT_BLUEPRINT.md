...
| 非人类调用 | `service_clients` 使用 `service_id + service_secret` 换取短期 `service_access`，服务 Principal 可参与 RBAC；client 显式为 platform 或单一 organization scope，组织 Token 每次实时重验组织与 membership | 当前没有直接 `X-API-Key` 鉴权、独立 device Principal 或多 key 生命周期；identity source 与 API key 的组织归属仍未实现 |
