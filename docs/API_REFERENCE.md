...
管理员通过更新接口将用户设为 `active: false` 时，Keylo 会撤销该用户全部 refresh session 并记录审计。带 Principal 的既有 access token 会在下一次受保护请求时被拒绝，不会继续等待其自然过期；已存在的 OIDC 浏览器 cookie 也不能再进入授权或同意流程。
