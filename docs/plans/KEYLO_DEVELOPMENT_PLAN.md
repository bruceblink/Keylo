...
| 验证 | 结果 |
| --- | --- |
| `.\scripts\run_tests.ps1 -DatabasePort 55432` | 使用本机 Docker `postgres:17-alpine`，宿主 `127.0.0.1:55432` 映射到容器 `5432`；PostgreSQL readiness、fmt、workspace Clippy、139 个单元、1 个 customer-support、25 个 database、76 个 HTTP、3 个 load、3 个 OAuth、12 个 RBAC 和 6 个 user 测试全部通过，脚本结束后容器、匿名卷、端口映射和临时密钥目录已清理。 |
...
| 顺序 | 功能 | 实现要求 | 验收结果 |
| --- | --- | --- | --- |
| 1 | 可插拔邮件投递边界 | 定义最小 mail provider 接口；测试使用 fake provider，生产接入受控 SMTP 或外部邮件服务；邮件正文和模板不进入认证核心权限判断。 | provider 超时或失败有稳定错误和审计结果，不改变既有登录安全边界。 |
| 2 | 用户邮箱验证 | 使用一次性、短时有效、服务端只保存摘要的 token；成功消费后更新 `email_verified`；修改邮箱立即清除旧验证状态。 | token 只能消费一次，过期、重放、未知用户和已修改邮箱均拒绝；审计不写入原 token。 |
| 3 | 忘记密码申请 | 以邮箱或用户名接收申请，但对已存在和不存在的账户返回相同公开结果；请求和发送均受限流保护。 | 无账户枚举；重复请求不会扩大有效 token 数量或绕过限流。 |
...
先冻结 token、限流、审计和会话撤销的数据约定，再按“邮件 provider -> 邮箱验证 -> 忘记密码 -> 密码重置 -> 文档与回归”顺序实现。每个可独立验收的步骤单独提交和推送，不把 SMTP、管理后台或其他协议扩展混入本切片。
