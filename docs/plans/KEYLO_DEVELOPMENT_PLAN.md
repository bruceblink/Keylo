...
| 2 | 用户邮箱验证 | 下一切片：使用一次性、短时有效、服务端只保存摘要的 token；成功消费后更新 `email_verified`；修改邮箱立即清除旧验证状态。 | token 只能消费一次，过期、重放、未知用户和已修改邮箱均拒绝；审计不写入原 token。 |
...
邮件 provider 边界已完成；后续按“邮箱验证 -> 忘记密码 -> 密码重置 -> 文档与回归”顺序实现。进入每个后续步骤前，先冻结该步骤的 token、限流、审计和会话撤销数据约定。每个可独立验收的步骤单独提交和推送，不把 SMTP、管理后台或其他协议扩展混入本切片。
