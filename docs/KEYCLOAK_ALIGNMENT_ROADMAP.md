...
当前主线准入条件：阶段 A/B 的标准 OIDC 协议契约、测试向量、四语言客户端样例、`scripts/validate_oidc_rp_examples.ps1` 构建回归和真实 HTTP 集成测试必须持续通过。Keycloak 标准测试 IdP 矩阵用于可选互操作回归，不因镜像、网络或部署条件不可用而阻塞轻量化主线。
