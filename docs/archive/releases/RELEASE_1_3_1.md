...
- `DATABASE_PASSWORD_ENC` / `DATABASE_PASSWORD_ENC_FILE`：默认读取 `./.secrets/.postgres_password.enc` 或 `/run/secrets/.postgres_password.enc`
- `DATABASE_PASSWORD_KEY` / `DATABASE_PASSWORD_KEY_FILE`：默认读取 `./.secrets/.database_password.key` 或 `/run/secrets/.database_password.key`
...
```env
DATABASE_PASSWORD_ENC_FILE=/run/secrets/.postgres_password.enc
DATABASE_PASSWORD_KEY_FILE=/run/secrets/.database_password.key
```
