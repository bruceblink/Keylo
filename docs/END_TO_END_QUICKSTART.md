...
Redis 默认不暴露到宿主机，只允许 Keylo 通过 Compose 内部网络访问。启动前需要准备 `.secrets/.redis.acl`、`.secrets/.redis_password.enc` 和 `.secrets/.redis_password.key`。

推荐用工具一次生成 Redis ACL hash、AES key 和 Redis 密码密文，过程中不会写入明文密码文件：

```bash
python scripts/secret_tool.py generate-redis
```

`.secrets/.redis.acl` 存 hash：
...
如果 Keylo 也在 Compose 中运行，`docker-compose.yml` 会把这两个文件挂载为 Docker secret。 如果你在宿主机上运行 `cargo run`，用本地开发 override 暴露 Redis 到回环地址：

```bash
docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d postgres redis
```

本机 `.env` 使用 `REDIS_HOST=127.0.0.1`、`REDIS_PORT=63790`、`REDIS_PASSWORD_ENC_FILE=./.secrets/.redis_password.enc`、`REDIS_PASSWORD_KEY_FILE=./.secrets/.redis_password.key`。如果要连接测试环境 Redis，只改 `REDIS_HOST` / `REDIS_PORT` 即可，不需要重新生成密文。
...
Keylo 会在启动早期校验数据库 URL、Token 时长等关键配置；缺少 RSA 密钥配置时会自动生成并持久化密钥文件。只有本地临时调试且明确接受数据库能力不可用时，才设置 `ALLOW_IN_MEMORY_FALLBACK=true`；该模式仍会校验基础配置。
...
- `Invalid startup configuration`
  - 检查密钥目录是否可写，或显式配置 `JWT_PRIVATE_KEY_PATH` / `JWT_PUBLIC_KEY_PATH`。
  - 数据库模式下检查 `DATABASE_URL`。
  - 生产环境检查 Redis 密码密文配置。

首次未完成 setup 时，日志会输出临时 setup token。访问 `http://127.0.0.1:2345/setup`，输入该 token、`ADMIN_CLIENT_ID` 和新的 `Admin Client Secret` 完成初始化。密钥只会以哈希形式写入数据库，不需要写入 `.env`。
...
首次 setup 初始化会根据页面提交的 `ADMIN_CLIENT_ID` / `Admin Client Secret` 创建一个管理客户端。
...
5. 容器启动后 `skipping automatic admin client seed`
   ...
   - 首次部署请从日志读取临时 setup token，进入 `/setup` 初始化管理客户端。
