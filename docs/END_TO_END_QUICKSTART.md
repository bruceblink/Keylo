...
推荐用工具一次生成 Redis ACL hash、AES key 和 Redis URL 密文，过程中不会写入明文密码文件：
...
`.secrets/.redis_url.enc` 存 AES-GCM 密文 URL，`.secrets/.redis_url.key` 存解密 key。Keylo 启动时只在内存中解密出明文 Redis URL 再连接 Redis。
...
并在本机 `.env` 使用 `REDIS_URL_ENC_FILE=./.secrets/.redis_url.local.enc`、`REDIS_URL_KEY_FILE=./.secrets/.redis_url.local.key`。
