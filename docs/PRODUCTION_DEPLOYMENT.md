...
- Redis 不发布宿主机端口，且只加入 Keylo 专用内部网络；生产环境 `REDIS_URL` 必须包含 ACL 用户名和密码。
- `DB_POOL_SIZE` 控制数据库连接池大小，默认 10；只有需要按部署规模调优时才覆盖。
- 如果数据库初始化失败，服务会直接失败启动，不再回退到内存模式。
- 非生产环境默认也会在数据库初始化失败时失败启动；只有显式设置 `ALLOW_IN_MEMORY_FALLBACK=true` 才允许无数据库路由。该模式仍会校验管理员客户端和其他基础配置。
...
生产环境必须启用 Redis ACL。推荐准备 `./secrets/redis.acl`：
